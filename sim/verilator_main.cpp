...
//#include "Vvproc_top_cv32e40x_wb_stage__Xz14.h"
#else
//#include "Vvproc_top_cv32e40x_wb_stage__Xz12.h"
#endif
...
int main(int argc, char **argv) {
    ...
    while (getline(&line, &line_sz, fprogs) > 0) {
        ...
        // simulate program execution
        {
            ...
            int cycles_stalled_cache_miss_data = 0; //TODO RENAME
            int cycles_stalled_i_d_mem_conflict = 0;

            int number_misaligned_ls = 0;
            ...
            while (end_cnt < extra_cycles) {
                ...
                // Fulfill request on the instruction memory port (only used if caches aren't enabled)
                ...
                //cycles spent loading from memory (is this correct???)
                if (mem_rvalid_queue[mem_latency-1] && main_reached)
                {
                    cycles_stalled_cache_miss_data++;
                }
               ...
               /* bool has_offloaded = top->vproc_top->has_offloaded;
                ...
                bool I_miss = top->vproc_top->i_miss;
                bool I_hit = top->vproc_top->i_hit;
                ...
                bool D_miss = top->vproc_top->d_miss;
                bool D_hit = top->vproc_top->d_hit;
                ...
                //bool mem_access_conflict = top->vproc_top->id_mem_collision;

                //if (mem_access_conflict && main_reached)
                //{
                    
                //    cycles_stalled_i_d_mem_conflict++;
               // }

                //bool misaligned_ls = top->vproc_top->v_core->dec->misaligned_ls;

                //if (misaligned_ls && main_reached)
                //{
                //    number_misaligned_ls++;
                //}
            ...
            fprintf(stderr, "Number of Misaligned vector Loads/Stores : %d\n", number_misaligned_ls);

            fprintf(stderr, "Cycles spent reading from memory : %d\n", cycles_stalled_cache_miss_data);

            fprintf(stderr, "Cycles spent stalling for memory access conflict : %d\n", cycles_stalled_i_d_mem_conflict);
